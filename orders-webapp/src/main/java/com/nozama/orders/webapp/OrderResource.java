...
public class OrderResource {
	...
	private Queue holdresponsequeue;
	private Queue holdrequestqueue;
	private ConnectionFactory holdrequestfactory;
	...
	public OrderResource(OrderService serv) {
		this.serv = serv;
		try {
			this.holdrequestqueue = InitialContext.doLookup("jms/HoldRequestQueue");
		} catch (NamingException e) {
			e.printStackTrace();
		}

		try {
			this.holdrequestfactory = InitialContext.doLookup("jms/HoldRequestFactory");
		} catch (NamingException e) {
			e.printStackTrace();
		}

		try {
			this.holdresponsequeue = InitialContext.doLookup("jms/HoldResponseQueue");
		} catch (NamingException e) {
			e.printStackTrace();
		}
		...
		try {
			this.checkouttopic = InitialContext.doLookup("jms/CheckoutTopic");
		} catch (NamingException e) {
			e.printStackTrace();
		}

		this.ctx = holdrequestfactory.createContext();
		this.producer = ctx.createProducer();
		this.consumer = ctx.createConsumer(holdresponsequeue);
	}
	...
	private HoldResponse getHoldResponse() {
		...
		while (hr == null) {
			hr = consumer.receiveBodyNoWait(HoldResponse.class);    //TODO: get the hold response from the hold response queue in a non-blocking fashion
